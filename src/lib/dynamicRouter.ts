import RouteLog from './routeLog.js';
import { blue, underline, red } from './colors.js';
import texts from '../constants/texts.js';
import allowedMethods from '../constants/allowedMethods.js';
...
class DynamicRouter {
	...
	run() {
		...
		if (this.port) {
			...
			this.app.listen(this.port, () => {
				console.log(`${texts.server_is_running} ${blue(`http://${ip}:${this.port}`)}`);
			});
		}
	...
	#loadRoutes() {
		for (const file of this.routerFiles) {
			this.#log(blue(underline(file)));

			let routes = [];
			try {
				routes = require(`${this.folders.routers}/${file}`);
			} catch (error) {
				console.log(red(`${file} ${texts.router_file_not_found}`));
				console.log();
				break;
			}
